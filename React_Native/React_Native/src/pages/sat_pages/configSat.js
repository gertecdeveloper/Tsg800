...
export default class ConfigSat extends Component{
    ...
    configRede(){
        ...
        if(validationCodigo.getValidation(this.state.codigoAtivacao)){
            // ToastExample.configRede(
            //     Math.floor(Math.random() * 9999999),
            //     this.state.codigoAtivacao,
            //     this.formatarEnvioConfigRede()
            // );
    ...
    formatarEnvioConfigRede(){
        ...
        if (this.state.tipoRede == "estatico") {
            ...
            if (this.state.ipSat!='') {
            dadosXml[1] = "<lanIP>" + this.state.ipSat + "</lanIP>";
            }
            if (this.state.mascara!='') {
            dadosXml[2] = "<lanMask>" + this.state.mascara + "</lanMask>";
            }
            if (this.state.gateway!='') {
            dadosXml[3] = "<lanGW>" + this.state.gateway+ "</lanGW>";
            }
    
            // Monta as tags do XML sobre a parte de DNS
            if (this.state.habilitarDNS == "sim") {
            if (this.state.dns1!='') {
                dadosXml[4] = "<lanDNS1>" + this.state.dns1 + "</lanDNS1>";
            }
            if (this.state.dns2!='') {
                dadosXml[5] = "<lanDNS2>" + this.state.dns2 + "</lanDNS2>";
            }
            } else {
        ...
        if (this.state.habilitarProxy == "nao usa") {
            ...
            if (this.state.proxyIp!='') {
            dadosXml[7] = "<proxy_ip>" + this.state.proxyIp + "</proxy_ip>";
            }
            if (this.state.porta!='') {
            dadosXml[8] = "<proxy_porta>" + this.state.porta + "</proxy_porta>";
            }
            if (this.state.user!='') {
            dadosXml[9] = "<proxy_user>" + this.state.user + "</proxy_user>";
            }
            if (this.state.password!='') {
            dadosXml[10] = "<proxy_senha>" + this.state.password+ "</proxy_senha>";
            }
        } else {
